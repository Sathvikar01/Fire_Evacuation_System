"""Hard-condition experiment runner with chunking support.

Usage:
  python run_hard_conditions.py --mode full_cms_daco --seed-start 1 --seed-end 15
  python run_hard_conditions.py --merge   # merges all hard_*_*.json into hard_conditions_final
"""
import argparse
import json
import os
...
from experiments.run_comprehensive_evaluation import (
    run_single, aggregate_results, mean_ci, save_results,
)
import config
...
CONFIGS = {
    "distance": {
        "mode": config.MOVEMENT_MODE_DISTANCE,
        "flags": {},
    },
    "astar": {
...
def run_chunk(mode_name, seed_start, seed_end):
    ...
    # CRITICAL: apply flags to config BEFORE any core import (fresh process).
    ...
    out = Path(__file__).parent / "results" / f"hard_{mode_name}_{seed_start}_{seed_end}.json"
    ...
    tmp.write_text(json.dumps({"mode": mode_name, "seed_start": seed_start,
                               "seed_end": seed_end, "results": results}, default=conv))
    tmp.replace(out)
...
def merge():
    ...
    print("\n" + "=" * 110)
    print(f"HARD CONDITIONS N={len(grouped['distance'])} {GRID}x{GRID} crowd={CROWD} "
          f"walls={WALLS} wind=east/0.5 blocked=1 ticks={TICKS}")
    print("=" * 110)
    ...
    payload = {
        "type": "hard_conditions",
        "config": {"grid": GRID, "crowd": CROWD, "exits": EXITS, "walls": WALLS,
                   "ticks": TICKS, "wind": "east/0.5", "blocked": 1},
        "results": grouped,
        "summary": summary,
    }
    save_results(payload, "hard_conditions_final")
