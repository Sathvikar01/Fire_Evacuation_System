...
from config import (
    ...
    COLOR_SMOKE, COLOR_CONGESTION, COLOR_PHEROMONE, COLOR_GRID_LINE,
    MOVEMENT_MODE_ACO,
...
class GridWidget(QWidget):
    ...
    def mousePressEvent(self, e):
        try:
            ...
            x, y = e.x(), e.y()
        r,c = self._pos_to_cell(x, y)
        if r is None: return
        self.apply_tool(r,c); self.update()
        self._last_drag_cell = (r,c)

    def mouseMoveEvent(self, e):
        if e.buttons() & Qt.LeftButton and self.tool:
            try:
                x, y = e.pos().x(), e.pos().y()
            except Exception:
                x, y = e.x(), e.y()
            r,c = self._pos_to_cell(x, y)
            if r is None: return
            if getattr(self, '_last_drag_cell', None) != (r,c):
                self.apply_tool(r,c)
                self._last_drag_cell = (r,c)
                self.update()
    ...
    def _pos_to_cell(self, x, y):
        ...
        cw = w / C; ch = h / R
        if cw <= 0 or ch <= 0: return None, None
    ...
    def paintEvent(self, _):
        ...
        try:
            ...
            # Pheromone overlay — optimized: solid alpha fill, correct (pher-FLOOR)/(max-FLOOR) normalization
            if self.show_pheromone and getattr(self.sim, 'movement_mode', None) == MOVEMENT_MODE_ACO:
                from config import PHEROMONE_FLOOR as _PF
                mxp = float(g.pheromone.max())
                mn = float(_PF)
                denom = max(1e-6, mxp - mn)
                for r in range(R):
                    for c in range(C):
                        v = float(g.pheromone[r, c])
                        if v <= mn + 0.02:
                            continue
                        raw = (v - mn) / denom
                        boosted = raw * PHEROMONE_VISIBILITY_SCALE
                        if boosted <= 0.02:
                            continue
                        if boosted > 1.0:
                            boosted = 1.0
                        ...
                        if level <= 0.02:
                            continue
                        rect = tile_rect(r, c)
                        overlay = QColor(*COLOR_PHEROMONE)
                        overlay.setAlpha(int(PHEROMONE_ALPHA_MAX * level * 0.75))
