...
\item \textbf{A controlled comparison} of six policy implementations (Random, BFS-greedy, Hazard-aware A*, D* Lite, Standard ACO, Full CMS-DACO) over 120 policy--environment cells with $N{=}30$ \emph{paired} seeds per cell; because layout, fire origin, and spawn derive from the seed index, all inference uses paired t-tests, Wilcoxon signed-rank tests, bootstrap CIs of paired differences, Cohen's $d_z$, and Holm-corrected families --- plus five-component ablations, four parameter sweeps, robustness sweeps (crowd, obstacles, wind, grid), a partial-observability study, an extreme-stress regime, and compute-budget matching.
...
\label{sec:scale}
To test whether the runtime argument changes before substantially larger environments are attempted, we ran a paired $N{=}20$ sweep on $30^2$ and $40^2$ grids with wall density $0.15$, one blocked exit, East wind $0.5$, and area-scaled crowds of 60 and 100. At $30^2$, completion was $0.871\pm0.038$ for Distance, $0.999\pm0.002$ for A*, $0.949\pm0.032$ for D* Lite, and $0.888\pm0.038$ for Full DACO; at $40^2$, the corresponding values were $0.796\pm0.045$, $0.996\pm0.008$, $0.935\pm0.026$, and $0.828\pm0.054$. A* remained the most reliable policy, while its measured runtime rose from $40.6$ to $166.9$ seconds per run and Full DACO from $206.6$ to $391.6$ seconds per run. Thus the first scale points show substantial absolute A* cost growth but no crossover: DACO also becomes slower and less reliable. These two points do not justify extrapolation to large maps, but they replace an unsupported runtime conjecture with measured evidence.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_scale_runtime.pdf}
\caption{Scale sweep ($N{=}20$ per point): completion and wall-clock seconds per run on $30^2$ and $40^2$ grids. A* remains more reliable and faster at both measured scales.}
\label{fig:scale}
\end{figure}
