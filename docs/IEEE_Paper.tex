...
\author{\IEEEauthorblockN{Sathvika R.} % TODO(author): verify full name/affiliation before submission
...
\begin{abstract}
Stigmergic routing (ant colony optimization, ACO) is widely proposed for building evacuation under spreading hazards, almost always against weak baselines. We present CMS-DACO, an open-source grid-world testbed coupling cellular pedestrian dynamics with a vectorized fire/smoke/wind engine, and use it to ask \emph{when}, if ever, cached pheromone guidance beats classical replanning. Five policies---random walk, greedy BFS-distance, per-tick hazard-aware A*, incremental D* Lite, and a full DACO stack (BFS-seeded pheromone, annealed ants, dual speed/safety channels, predictive congestion)---are compared under matched information with $N{=}30$ paired seeds per cell; we report 95\% CIs, paired t-tests, Wilcoxon signed-rank tests, bootstrap CIs of paired differences, and Holm-corrected families. Four regimes are probed: open and degraded maps, \emph{partial observability} (local sensing radius, stale maps, 50\% observation loss), \emph{extreme stress} (20\% walls, two fire fronts, exits collapsing mid-run), and \emph{compute-budget-matched} ACO. Results are unambiguous. Directed policies beat random walking by $d\geq11$. Per-tick A* dominates the full pheromone stack in every visibility regime ($|\bar{d}|=5$--$6$ points, Holm $p\leq0.002$, $d_z\geq0.66$); partial observability does not flip the ordering because soft hazard costs degrade gracefully on small maps while stigmergy keeps paying detour costs. Under extreme stress completion collapses to $0.40$--$0.53$ for everyone and the ordering \emph{reverses}: greedy BFS-distance significantly outperforms A* (Holm $p{=}0.049$), which over-commits agents to an exit that later burns. Cutting ACO compute $10\times$ (182$\to$19 s/run) leaves its quality unchanged, so budget-matching cannot rescue it. We conclude that on fully observable small grids---and even under sensory deprivation there---stigmergy purchases nothing over replanning at any compute level; its remaining promise lies in regimes this study deliberately bounds out (large partial maps, communication-limited swarms), for which the released harness provides direct instrumentation.
\end{abstract}
...
Planners read \emph{belief} fields $\tilde{f},\tilde{s}$ instead of ground truth; physics (fire growth, casualties) always uses truth. Three degradation mechanisms isolate different information failures:
\begin{itemize}
...
Default hyperparameters were fixed a priori from literature-plausible ranges during development on the open family; Section~VII-C shows flat sweeps, so results do not hinge on tuning (the circularity of this defense is noted in Sec.~VIII).
...
\label{sec:obs}
Fig.~\ref{fig:observability} and Table~\ref{tab:obs} give the central boundary-condition result on the hard map. A* is essentially \emph{insensitive} to sensory deprivation: completion stays at $0.991$--$0.994$ under local sensing ($r{=}3$), 5-tick-stale maps, and 50\% observation loss. The reason is structural---hazard terms enter planner costs as \emph{soft} penalties on a $15^2$ map whose alternative corridors differ by only a few ticks, so even badly wrong beliefs rarely change the argmin corridor; meanwhile adjacent-cell candidate filtering (shared by all policies) already prevents most fatal steps. Full CMS-DACO likewise barely moves ($0.930$--$0.940$): its pheromone field was never relying on instantaneous global knowledge to begin with.

The consequence is that the stigmergic advantage hypothesized under degraded information does not materialize: paired DACO-vs-A* gaps remain $-5.3$ to $-6.3$ points in every regime (Holm $p\leq0.0022$, $d_z=0.66$--$0.85$, Wilcoxon $p\leq0.0007$). D* Lite tracks A* closely under degradation ($0.986$--$0.994$) though it starts lower with full knowledge ($0.957$, wide CI) and its extreme-regime variant hits an implementation ceiling noted below.
...
\textbf{Answering the boundary-condition question.} The four degradations tested here---worse maps, sensory deprivation (radius, staleness, loss), extreme dynamism, and compute starvation---span the regimes most commonly invoked to justify stigmergy. In none of them does the pheromone stack beat replanning: partial observability leaves A* untouched because soft hazard costs on small maps rarely change the argmin corridor; extreme stress rewards \emph{flow hedging}, which greedy distance provides for free and pheromones only partially; and quality is invariant to ant compute. The honest conclusion is that on fully observable, small-to-medium grids with per-tick replanning available, cached trail guidance is dominated---not merely outperformed at one operating point.
...
\textbf{When might stigmergy still matter?} Trails are written by agents and readable without a central solver or fresh global sensing. The remaining candidates are large maps where per-tick all-agents Dijkstra is genuinely expensive, communication-banned swarms, and heterogeneous sensing. Our harness instruments exactly these knobs (per-policy belief masks, sensor radii, map size); scaling beyond $15^2$--$20^2$ is the designated next experiment, and nothing in this paper should be read as foreclosing it.

\textbf{Threats.} (i) Grid abstraction and synchronous ticks abstract away continuous-space effects; scale is limited to $\leq20^2$, 45 agents. (ii) The hazard field is a lightweight cellular model calibrated for stress generation, not validated against FDS/CFAST---absolute survival rates must not be read as fire-engineering predictions. (iii) Single map-size family beyond one $20^2$ robustness point; single-machine timing. (iv) $N{=}30$ paired seeds resolve $d_z\gtrsim0.5$ comfortably but cannot exclude sub-point differences; Holm families are per-section. (v) Hyperparameters were selected informally on the open family before formal evaluation; the flat sweeps mitigate but do not eliminate this circularity. (vi) Our D* Lite caps expansions at 600/tick and uses a shared-belief cost model; stronger implementations could close its extreme-regime gap, though A*'s own reversal there suggests conclusions about replanning-class policies are robust to this choice.

\section{Conclusion}
Across open, degraded, partially observable, extremely dynamic, and compute-starved regimes---120 policy-environment cells, 3{,}600+ paired simulation runs---the empirical answer to ``when does stigmergic routing help evacuation?'' is: \emph{not in any regime tested}. Replanning dominates wherever information is sufficient for it to be computed in time; where information fails or the world turns adversarial, simple hedged heuristics rather than stigmergy capture the robustness margin. CMS-DACO's lasting contribution is therefore the benchmark itself: matched-information baselines up to and including incremental search, paired-seed statistics, ceiling-breaking stress design, and budget-matched fairness---packaged so that the next claim of ACO superiority can be tested in minutes rather than asserted.

\section*{Reproducibility}
All code, raw per-seed results, merged summaries, statistics, and figures are
released at
\href{https://github.com/Sathvikar01/Fire_Evacuation_System}{\texttt{github.com/Sathvikar01/Fire\_Evacuation\_System}}.
...
statistics via \texttt{compute\_stats\_paired.py}; figures via
\texttt{make\_figures.py}. Every table/figure in this paper regenerates from
stored JSON artifacts (3{,}600+ runs shipped).
